...
# Respuesta: Survived Percentage 38.38%

# 3. Calcular el porcentaje de missing values en cada uno de los atributos.
# Pista: averiguar que devuelve la funcion is.na cuando se aplica a un dataframe.

apply(titanic, 2, function(x) sum(is.na(x)))/length(titanic) # porcentaje de missing values
colSums(is.na(titanic))/length(titanic) # Otra forma de calcular el procentaje de Missing values
...
# 6. Calcular la probabilidad de supervivencia en base al genero. ¿Que
# conclusion(es) obtienes del resultado? Pista: dado que la variable
# survived toma el valor 1 cuando el pasajero sobrevivio, es sencillo
# calcular la probabilidad de supervivencia con una funcion estadistica.
...
# Respueta: Female: 74.20%, Male: 18.89%
...
# 7. Calcular la probabilidad de supervivencia en base a la edad. ¿Te parecen 
# facilmente interpretables estos resultados
...
#Respuesta: Probabilidad Superviviencia menores a 30 años: 37.07%; Probabilidad Superviviencia mayores a 30 años: 40.60%

# 8. Crea una nueva variable decade en el dataframe que contenga la decada de la edad 
# de los pasajeros y repite el analisis del apartado g) sobre esta nueva variable. 
# ¿Que conclusion(es) obtienes del resultado?
...
# Respuesta: para este caso tenemos la probabilidad de sobrevivir,
# por decadas de edad, lo cual nos permite distinguir entre grupos de edades.
# en este caso tenemos que hay mayor probabilidad de superviviencia para personas con
# menor a 10 años y para personas entre 70 y 80 años, (sin embargo la poblacion no es
# significativa)

# 9. Calcula la probabilidad de supervivencia en base a la clase del billete
# del pasajero (pclass). ¿Que conclusion(es) obtienes del resultado?
...
sum(titanic$Pclass[titanic$Survived==1 & titanic$Pclass==1])/nrow(titanic)
...
# La probabilidad de superviviencia de la clase 1: 62.96%, Clase 2: 47.28%, Clase 3: 24.23%


# 10. Combina en una tabla el analisis de la probabilidad de supervivencia
# en base al sexo y clase del billete del pasajero. ¿Que conclusiones se obtienen?

tapply(titanic$Survived, INDEX = list(titanic$Sex, titanic$Pclass), mean)

#Respuesta, prob superviviencia de las variables sexo y clase:

#Sex              1         2         3
#female 10.213244  7.856341  8.080808
#male    5.050505  1.907969  5.274972
...
# - sigleton: valor logico indicando con valor TRUE si el pasajero
# viaja solo y FALSE en caso contrario.

titanic$familysize <- titanic$Parch + titanic$SibSp +1
titanic$sigleron <- if_else(titanic$familysize==1, 1 , 0)

# 12. Calcula la probabilidad de supervivencia en base a si el pasajero viajaba
# solo o no. ¿Que conclusion se puede obtener?
...
# 13. Cuenta el numero de pasajeros por tamaño de familia y clase. Por
# ejemplo, cuantos pasajeros de primera clase pertenecen a una familia
# de tamaño 4. El resultado debe ser una matriz con la informacion
# para todas las posibles combinaciones de clase del billete y tamaño de familia.
...
# 15. Para unicar estos dos datraframes, parecera buena opcion utilizar
# la variable name como clave. Determina si esta variable es unica por
# pasajero utilizando la funcion duplicated o unique y mostrando el
# numero de nombres diferentes repetidos. En caso de existir varios
# pasajeros con el mismo nombre, listar aquellas las del dataframe inicial
# en las que el nombre del pasajero este repetido (la funcion %in% 
# puede resultar util para ello). De acuerdo a los resultados, ¿sera la
# combinacion del nombre del pasajero y la clase una buena clave para
# combinar los dataframes? Combina ambos dataframes utilizando la
# combinacion del nombre y el numero de billete respetando el orden de
# los dataframes de partida. Pista: funcion merge.

titanic3 <- merge(x = titanic, y = titanic2, by.x =c('Name','Ticket'),by.y =c('name','ticket'), all.x = TRUE)

head(titanic3)

# 16. ¿Qué porcentaje de los pasajeros que sobrevivió tiene asociado un identificador del
# bote salvavidas?
...
# que estan codicados como -9999.00.
...
diabetes <- diabetes[diabetes$BMI>0 & diabetes$BP>0 & diabetes$S2>0 & diabetes$S5>0 ,] #343

# 2. Ver el tipo de cada una de las variables.

str(diabetes) #tipo de variable de cada columna

# 3. Realizar un analisis estadistico de las variables numericas: calcular la
# media, varianza, rangos, etc. ¿Tienen las distintas variables rangos
# muy diferentes?.
...
# 4. Hacer un grafico de cajas (boxplot) donde se pueda ver la informacion
# anterior de forma grafica.

ggplot(stack(diabetes2),aes(x=ind,y=values))+geom_boxplot(aes(fill=ind),outlier.colour = 'Red')


# 5. Calcular la media para las filas que tienen SEX=M y la media para
# las filas que tienen SEX=F, utilizando la funcion tapply.
...
#Group.1      BMI       BP       S1       S2       S3       S4       S5       S6        Y
#1       F 26.80099 98.17562 190.6552 120.1103 44.54187 4.537882 4.731533 93.86207 155.8079
#2       M 25.95609 91.53474 188.0304 111.1726 54.56304 3.658217 4.569392 88.94348 149.0391
...
# 7. Realizar un grafico de dispersion para las variables que tienen mas y
# menos correlacion con Y y comentar los resultados. ¿Como seria el
# grafico de dispersion entre dos vectores con correlacion 1?.
...
# utilizando, por ejemplo, la codicacion M=1 y F=2.

diabetes$Sex2 <- as.numeric(factor(diabetes$SEX,levels = c('M','F')))


# 9. Definimos los outliers como los elementos (filas) de los datos para
# los que cualquiera de las variables (numericas) esta por encima o por
# debajo de la mediana mas/menos 3 veces el MAD (Median Absolute 
# Deviation). Identificar estos outliers y quitarlos.
...
#Respuesta: Al poner un parametro de corte de la madiana + 3 veces el mad, para cada variable, podriamos
# quitar algunos (no todos) los outliers del cada variabñe, es decir: persiten los outliers en algunas
# variables como se pede ver en los Bloxplots.


# 10. Separar el conjunto de datos en dos subconjuntos disjuntos de forma
...
# 11. Escalar los datos para que tengan media 0 y varianza 1, es decir, restar
# a cada variable numerica su media y dividir por la desviacion típica.
...
# Calcular la media y desviacion en el conjunto de train, y utilizar esa
# misma media y desviacion para escalar el conjunto de test.
